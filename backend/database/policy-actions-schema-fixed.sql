-- ==================================================
-- POLICY ACTIONS SUPPORT SCHEMA (FIXED VERSION)
-- Missing tables and columns for full policy action functionality
-- Compatible with existing users table constraints
-- ==================================================

-- First, let's safely modify the role constraint to support new values
DO $$
BEGIN
  ...
  -- Add the new constraint with expanded role values
...
-- Set default role for existing users without breaking constraints
UPDATE users SET role = 'admin' WHERE role IS NULL AND (email LIKE '%admin%' OR email LIKE '%security%');
...
-- Sample incident (will be created if table is new)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM incidents WHERE title = 'Policy Actions Schema Test') THEN
    INSERT INTO incidents (title, description, severity, status, source_type, employee_id, escalation_level) 
    VALUES ('Policy Actions Schema Test', 'Testing policy actions schema deployment', 'Low', 'Open', 'manual', 
